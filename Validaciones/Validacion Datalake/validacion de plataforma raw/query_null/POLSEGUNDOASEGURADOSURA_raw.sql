...
,SUM(CASE WHEN TRIM(UPPER(PROD.NUMEROPOLIZAPADREINPUT)) IS NULL AND TRIM(UPPER(UAT.NUMEROPOLIZAPADREINPUT))="" THEN 1 ELSE 0 END) AS DIFF_NUMEROPOLIZAPADREINPUT
,SUM(CASE WHEN TRIM(UPPER(PROD.NUMEROPOLIZAPADREVALUE)) IS NULL AND TRIM(UPPER(UAT.NUMEROPOLIZAPADREVALUE))="" THEN 1 ELSE 0 END) AS DIFF_NUMEROPOLIZAPADREVALUE
FROM iter-data-storage-pv-uat.acsele_data.POLSEGUNDOASEGURADOSURA_raw UAT 
INNER JOIN interseguro-data.acsele_data.POLSEGUNDOASEGURADOSURA_raw PROD 
ON UAT.PK = PROD.PK