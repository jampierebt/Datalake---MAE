...
,SUM(CASE WHEN TRIM(UPPER(PROD.DURACIONPOLINPUT)) IS NULL AND TRIM(UPPER(UAT.DURACIONPOLINPUT))="" THEN 1 ELSE 0 END) AS DIFF_DURACIONPOLINPUT
,SUM(CASE WHEN TRIM(UPPER(PROD.DURACIONPOLVALUE)) IS NULL AND TRIM(UPPER(UAT.DURACIONPOLVALUE))="" THEN 1 ELSE 0 END) AS DIFF_DURACIONPOLVALUE
FROM iter-data-storage-pv-uat.acsele_data.POLVIDATEMPORAL_raw UAT 
INNER JOIN interseguro-data.acsele_data.POLVIDATEMPORAL_raw PROD 
ON UAT.PK = PROD.PK