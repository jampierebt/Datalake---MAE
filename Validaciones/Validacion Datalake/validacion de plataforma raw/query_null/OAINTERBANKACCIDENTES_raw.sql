SELECT SUM(CASE WHEN TRIM(UPPER(PROD.OPCIONPLANIAINPUT)) IS NULL AND TRIM(UPPER(UAT.OPCIONPLANIAINPUT))="" THEN 1 ELSE 0 END) AS DIFF_OPCIONPLANIAINPUT
...
FROM iter-data-storage-pv-uat.acsele_data.OAINTERBANKACCIDENTES_raw UAT 
INNER JOIN interseguro-data.acsele_data.OAINTERBANKACCIDENTES_raw PROD 
ON UAT.PK = PROD.PK