...
,SUM(CASE WHEN TRIM(UPPER(PROD.NUMTIENDAINPUT)) IS NULL AND TRIM(UPPER(UAT.NUMTIENDAINPUT))="" THEN 1 ELSE 0 END) AS DIFF_NUMTIENDAINPUT
,SUM(CASE WHEN TRIM(UPPER(PROD.NUMTIENDAVALUE)) IS NULL AND TRIM(UPPER(UAT.NUMTIENDAVALUE))="" THEN 1 ELSE 0 END) AS DIFF_NUMTIENDAVALUE
,SUM(CASE WHEN TRIM(UPPER(PROD.FECHAULTIMAFACTURACIONINPUT)) IS NULL AND TRIM(UPPER(UAT.FECHAULTIMAFACTURACIONINPUT))="" THEN 1 ELSE 0 END) AS DIFF_FECHAULTIMAFACTURACIONINPUT
,SUM(CASE WHEN TRIM(UPPER(PROD.FECHAULTIMAFACTURACIONVALUE)) IS NULL AND TRIM(UPPER(UAT.FECHAULTIMAFACTURACIONVALUE))="" THEN 1 ELSE 0 END) AS DIFF_FECHAULTIMAFACTURACIONVALUE
...
FROM iter-data-storage-pv-uat.acsele_data.POLACCIDENTEASISTEN_raw UAT 
INNER JOIN interseguro-data.acsele_data.POLACCIDENTEASISTEN_raw PROD 
ON UAT.PK = PROD.PK