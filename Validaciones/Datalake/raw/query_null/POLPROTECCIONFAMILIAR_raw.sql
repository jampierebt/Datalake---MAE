...
,SUM(CASE WHEN TRIM(UPPER(PROD.PLANFAMILIARINPUT)) IS NULL AND TRIM(UPPER(UAT.PLANFAMILIARINPUT))="" THEN 1 ELSE 0 END) AS DIFF_PLANFAMILIARINPUT
,SUM(CASE WHEN TRIM(UPPER(PROD.PLANFAMILIARVALUE)) IS NULL AND TRIM(UPPER(UAT.PLANFAMILIARVALUE))="" THEN 1 ELSE 0 END) AS DIFF_PLANFAMILIARVALUE
...
FROM iter-data-storage-pv-uat.acsele_data.POLPROTECCIONFAMILIAR_raw UAT 
INNER JOIN interseguro-data.acsele_data.POLPROTECCIONFAMILIAR_raw PROD 
ON UAT.PK = PROD.PK