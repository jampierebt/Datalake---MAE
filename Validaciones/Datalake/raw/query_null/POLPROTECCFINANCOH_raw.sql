...
,SUM(CASE WHEN TRIM(UPPER(PROD.PLANPROTECCFINANCOHINPUT)) IS NULL AND TRIM(UPPER(UAT.PLANPROTECCFINANCOHINPUT))="" THEN 1 ELSE 0 END) AS DIFF_PLANPROTECCFINANCOHINPUT
,SUM(CASE WHEN TRIM(UPPER(PROD.PLANPROTECCFINANCOHVALUE)) IS NULL AND TRIM(UPPER(UAT.PLANPROTECCFINANCOHVALUE))="" THEN 1 ELSE 0 END) AS DIFF_PLANPROTECCFINANCOHVALUE
FROM iter-data-storage-pv-uat.acsele_data.POLPROTECCFINANCOH_raw UAT 
INNER JOIN interseguro-data.acsele_data.POLPROTECCFINANCOH_raw PROD 
ON UAT.PK = PROD.PK