...
CREATE OR REPLACE TABLE `interseguro-data.temp.TMP_POLIZA_CARGO_ABONO_EXPORT_GCP_PRUEBA`
...
--,PCA.FECHA_MODIFICACION_REGISTRO
 FROM `interseguro-data.goldenrecord_data.POLIZA_CARGO_ABONO` PCA
...
-- ------------------------------------------------------------------------------------
EXPORT DATA OPTIONS(
URI='gs://interseguro-datalake-prd-landing/PRD/ODS/POLIZA_CARGO_ABONO/POLIZA_CARGO_ABONO-*.csv.gz',
FORMAT='CSV',
COMPRESSION='GZIP',
overwrite=true,
header=false,
...
,CATEGORIA_AGENTE
--FROM `interseguro-data.goldenrecord_data.POLIZA_CARGO_ABONO`
FROM  `interseguro-data.temp.TMP_POLIZA_CARGO_ABONO_EXPORT_GCP_PRUEBA`
--WHERE NOMBRE_GRUPO_PRODUCTO IN('VIDA','VEHICULAR')
--WHERE NOMBRE_GRUPO_PRODUCTO ='VEHICULAR'

;

DROP TABLE  IF EXISTS `interseguro-data.temp.TMP_POLIZA_CARGO_ABONO_EXPORT_GCP_PRUEBA`;
