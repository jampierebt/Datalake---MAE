...
-- ------------------------------------------------------------------------------------
EXPORT DATA OPTIONS(
URI='gs://interseguro-datalake-prd-landing/PRD/ODS/POLIZA_OPERACION_ASEGURADO/POLIZA_OPERACION_ASEGURADO-*.csv.gz',
FORMAT='CSV',
COMPRESSION='GZIP',
overwrite=true,
header=false,
...
FROM `interseguro-data.goldenrecord_data.POLIZA_OPERACION` A
WHERE
...
SELECT p.numero_poliza,
   ...
   FROM poliza_operacion p
     LEFT JOIN `interseguro-data.goldenrecord_data.ASEGURADO` a ON p.numero_poliza = a.numero_poliza
