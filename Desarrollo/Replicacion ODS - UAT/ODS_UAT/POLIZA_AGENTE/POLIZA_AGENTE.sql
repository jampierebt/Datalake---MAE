CREATE OR REPLACE TABLE `iter-data-storage-pv-uat.temp.TMP_POLIZA_AGENTE_GCP_PRUEBA`
AS
WITH  TMP_AGENTE AS (
SELECT
COD_AGENTE
...
FROM  `iter-data-storage-pv-uat.goldenrecord_data.MM_ESTRUCTURA_COMERCIAL` A --> 202305
LEFT JOIN `iter-data-storage-pv-uat.goldenrecord_data.MM_ESTRUCTURA_COMERCIAL` B -->
...
FROM `iter-data-storage-pv-uat.goldenrecord_data.POLIZA_AGENTE` A
...
-- ------------------------------------------------------------------------------------
EXPORT DATA OPTIONS(
URI='gs://interseguro-datalake-prd-landing/UAT/ODS/POLIZA_AGENTE/POLIZA_AGENTE-*.csv.gz',
FORMAT='CSV',
COMPRESSION='GZIP',
overwrite=true,
header=false,
...
FROM `iter-data-storage-pv-uat.temp.TMP_POLIZA_AGENTE_GCP_PRUEBA`;

-- ------------------------------------------------------------------------------------
-- Paso 4: Delete a temporales usadas
-- ------------------------------------------------------------------------------------
DROP TABLE IF EXISTS`iter-data-storage-pv-uat.temp.TMP_POLIZA_AGENTE_GCP_PRUEBA` ;
