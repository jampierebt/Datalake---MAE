CREATE OR REPLACE TABLE `interseguro-data.temp.TMP_POLIZA_BETTERFLY_UNIVERSO_GCP`
AS
...
FROM `interseguro-data.goldenrecord_data.POLIZA_COBERTURA` POL
...
, 'Perú' AS pais
, POL.plan
FROM `interseguro-data.goldenrecord_data.POLIZA` POL
LEFT JOIN `interseguro-data.goldenrecord_data.ASEGURADO` ASE
...
-- ------------------------------------------------------------------------------------
EXPORT DATA OPTIONS(
URI='gs://interseguro-datalake-prd-landing/PRD/ODS/POLIZA_BETTERFLY/POLIZA_BETTERFLY-*.csv.gz',
FORMAT='CSV',
COMPRESSION='GZIP',
overwrite=true,
header=false,
...
FROM `interseguro-data.temp.TMP_POLIZA_BETTERFLY_UNIVERSO_GCP`
;


DROP TABLE IF EXISTS `interseguro-data.temp.TMP_POLIZA_BETTERFLY_UNIVERSO_GCP` ;
