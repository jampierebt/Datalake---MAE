...
-- ------------------------------------------------------------------------------------
CREATE OR REPLACE TABLE `interseguro-data.temp.PLANILLA_SOAT`
AS 
...
from   `interseguro-data.samp_data.PLANILLA_raw` p
       left join `interseguro-data.samp_data.APLICACION_RECIBO_CAJA_raw` arc on p.id_planilla = arc.id_planilla_aplicado
...
-- ------------------------------------------------------------------------------------
EXPORT DATA OPTIONS(
URI='gs://interseguro-datalake-prd-landing/PRD/ODS/PLANILLA_SOAT/PLANILLA_SOAT-*.csv.gz',
FORMAT='CSV',
COMPRESSION='GZIP',
overwrite=true,
header=false,
...
monto_pagado,
estado_homologado
FROM `interseguro-data.temp.PLANILLA_SOAT`;
