...
# Nombre de la tabla
PRM_NOMBRE_TABLA=POLIZA_AGENTE
...
PRM_POSGRESQL_SERV_BDANALITICA_DEV=`grep -i PRM_POSGRESQL_SERV_BDANALITICA_DEV ${PRM_ARCHIVO_CONFIG} | awk -F= '{printf("%s",$2)}'`
PRM_POSGRESQL_USER_BDANALITICA_DEV=`grep -i PRM_POSGRESQL_USER_BDANALITICA_DEV ${PRM_ARCHIVO_CONFIG} | awk -F= '{printf("%s",$2)}'`
PRM_POSGRESQL_PWD_BDANALITICA_DEV=`grep -i PRM_POSGRESQL_PWD_BDANALITICA_DEV ${PRM_ARCHIVO_CONFIG} | awk -F= '{printf("%s",$2)}'`
PRM_POSGRESQL_BD_BDANALITICA_DEV=`grep -i PRM_POSGRESQL_BD_BDANALITICA_DEV ${PRM_ARCHIVO_CONFIG} | awk -F= '{printf("%s",$2)}'`
...
psql "host=${PRM_POSGRESQL_SERV_BDANALITICA_DEV} port=5432 dbname=${PRM_POSGRESQL_BD_BDANALITICA_DEV} user=${PRM_POSGRESQL_USER_BDANALITICA_DEV} password=${PRM_POSGRESQL_PWD_BDANALITICA_DEV}" <<EOF_1

CALL  staging.sp_poliza_agente();
